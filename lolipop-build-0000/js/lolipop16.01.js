...
(function(){
	...
	Data.Formatting.Functions.ultraAbbreviated = function( bn ){
		if( bn.gte(1) ){
			...
			console.log('TO FIXED',bn.toFixed(2));
	...
	(function(){
		...
		Game.save = function(){
			...
			Data.Pedia.MonsterIDs.map( function(str){
				saveVar(obj,'monster.'+str+'.quantity')}
			);
			makeUpVar(obj,'action.time.elapsed', Date.now());

			console.log('SAVE',obj);
			localStorage.setItem('game.save',JSON.stringify(obj));
		}

		Game.load = function(){
			var obj = JSON.parse(localStorage.getItem("game.save"));
			if( obj ){
				console.log(obj);
				for( var key in obj ){
					var val = obj[key];
	...
	Loliprivate.clickOnMick = function(){
		...
		Data.updateVar('action.click.mick');
	}
	...
	Upgrade.Pedia = { 'C20' : { targets: ['game.gold.gpc'], op: '*', value: new BN('2'), unlockedBy: [Milestone('statistics.mick.clicksAllTime',new BN(25))], shop: { price: new BN(50), name: 'Smashing Click', description: 'The amount of gold earned by clicking the minion is <strong>doubled</strong>.', image: 'smash1' }},'C21' : { targets: ['game.gold.gpc'], op: '*', value: new BN('2'), unlockedBy: [Milestone('statistics.mick.clicksAllTime',new BN(100))], shop: { price: new BN(200), name: 'Lucrative Smash', description: 'The amount of gold earned by clicking the minion is <strong>doubled</strong>.', image: 'smash2' }},'C22' : { targets: ['game.gold.gpc'], op: '*', value: new BN('2'), unlockedBy: [Milestone('statistics.mick.clicksAllTime',new BN(300))], shop: { price: new BN(1500), name: 'Super Smash', description: 'The amount of gold earned by clicking the minion is <strong>doubled</strong>.', image: 'smash3' }},'P00' : { targets: ['constant.monster.poro.gps'], op: '+', value: new BN('0.1'), unlockedBy: [Milestone('monster.poro.quantity',new BN(5))], shop: { price: new BN(40), name: 'Fluffy Group', description: 'Being a group allow each poro to find <strong>+0.1</strong> gold per second.', image: 'poro1' }},'P01' : { targets: ['constant.monster.poro.gps'], op: '*', value: new BN('2'), unlockedBy: [Milestone('monster.poro.quantity',new BN(10))], shop: { price: new BN(300), name: 'Thick Furr', description: 'Poros can find gold <strong>twice</strong> as fast.', image: 'poro1' }},'P02' : { targets: ['constant.monster.poro.gps'], op: '*', value: new BN('2'), unlockedBy: [Milestone('monster.poro.quantity',new BN(30))], shop: { price: new BN(5000), name: 'Magma Furr', description: 'Poros can find gold <strong>twice</strong> as fast.', image: 'poro2' }},'P03' : { targets: ['constant.monster.poro.gps'], op: '*', value: new BN('2'), unlockedBy: [Milestone('monster.poro.quantity',new BN(75))], shop: { price: new BN(1000000), name: 'Fantom Furr', description: 'Poros can find gold <strong>twice</strong> as fast.', image: 'poro3' }},'W00' : { targets: ['constant.monster.wolf.gps'], op: '+', value: new BN('0.5'), unlockedBy: [Milestone('monster.wolf.quantity',new BN(5))], shop: { price: new BN(400), name: 'Wild Nose', description: 'Wolves now have a better sense of smell and find gold <strong>twice</strong> as fast.', image: 'wolf2' }},'W01' : { targets: ['constant.monster.wolf.gps'], op: '*', value: new BN('2'), unlockedBy: [Milestone('monster.wolf.quantity',new BN(10))], shop: { price: new BN(1500), name: 'Wild Nose', description: 'Wolves now have a better sense of smell and find gold <strong>twice</strong> as fast.', image: 'wolf1' }},'R00' : { targets: ['constant.monster.raptor.gps'], op: '+', value: new BN('2'), unlockedBy: [Milestone('monster.raptor.quantity',new BN(5))], shop: { price: new BN(2000), name: 'Red Raptors', description: 'New species of raptors increase the chances of getting golden eggs. #Diversity', image: 'raptor1' }},'R01' : { targets: ['constant.monster.raptor.gps'], op: '*', value: new BN('2'), unlockedBy: [Milestone('monster.raptor.quantity',new BN(10))], shop: { price: new BN(7000), name: 'Red Raptors', description: 'New species of raptors increase the chances of getting golden eggs. #Diversity', image: 'raptor1' }},'XX0' : { targets: ['game.gold.gps'], op: '*', value: new BN('100000'), unlockedBy: [Milestone('game.gold.bank',new BN(0))], shop: { price: new BN(0), name: 'CHEAT', description: 'CHEAAAAT GPS', image: 'poro8' }},'XX1' : { targets: ['game.gold.gpc'], op: '*', value: new BN('100000'), unlockedBy: [Milestone('game.gold.bank',new BN(0))], shop: { price: new BN(0), name: 'CHEAT', description: 'CHEAAAAT GPC', image: 'poro7' }} };
	...
	Data.registerConstant("game.gold.gpc", new BN(1));

	// Gold earned per second
	Data.registerRelayVar("game.gold.gps", ['monster.gps']);

	Data.registerProductVar("action.click.goldProduced",['action.click.mick'],['game.gold.gpc'], true);
	...
	Interface.actions.closeMainMenu = function(){

	}

	Interface.actions.openMonsterWindow = function(){
	...
	Interface.View.change = function( id ){
		...
		if( Interface.View.list[id] ){
			if(Interface.View.current) Interface.View.current.close();
			Interface.View.current = Interface.View.list[id];
			Interface.View.current.open();
	...
	Interface.interact = function( elt ){
		...
		codes = codes.split(',');
		codes = codes.map(function( code ){ var tab = code.match(/([a-z]+):(.*)/); if(tab && tab[1] && tab[2]) return {cat:tab[1],val:tab[2]}; }).filter(function(o){return o?true:false});
		
		for( var i in codes ){
			var interaction = codes[i];

			switch(interaction.cat){
				case 'view': Interface.View.change(interaction.val); break;
				case 'action': 
					if( interaction.val !== 'action.click.mick') Data.updateVar(interaction.val); break;
			}
		}
	};
	...
	Interface.View.list['main-menu'] = {
		open: function(){
			$('#main-menu').removeClass('hide-left');
	...
	Interface.View.list['monster-list'] = {
		open: function(){
			console.log('Opening monster-list');
			$('#windows-insertion').append($("\
				<div id='monster-list' class='horizontal-hide hide-right'></div>\
			"));

			var monsters = Data.Pedia.Monsters, container = $('#monster-list');

			/*container.append($('\
					<div id="item-back" class="item" onClick="Lolipop.interact(this);" data-lolipop="view:back">\
						<div class="thumbnail"  style=\'background-image:url("../img/mobile/back.png?v=2");\'></div>\
			...
			for( var m in monsters ){
				...
				container.append($('\
					<div id="item-'+m+'" class="item conditional-format" data-condition="'+monster.vars.unaffordable+'" data-format-class="unaffordable">\
						<div class="click-zone"></div>\
						<div class="thumbnail"  style=\'background-image:url("../img/mobile/monster-'+monster.code+'.jpg?v=2");\'></div>\
						<div class="quantity gamedata" data-format-function="ultraAbbreviated" data-id="'+ monster.vars.quantity +'">ERR</div>\
						<div class="title">'
							+monster.SummonMenu.name+
						'</div>\
						<div class="menu adjust-reveal"><div><div>\
							<div id="item-'+m+'-1" class="small item conditional-format" data-condition="'+monster.vars.unaffordable+'" data-format-class="unaffordable" onClick="Lolipop.interact(this);" data-lolipop=action:'+monster.vars.actionBuy+'>\
								<div class="filler">1</div>\
								<div class="title gamedata" data-format-function="ultraAbbreviated" data-id="'+ monster.vars.price +'">'
									+'156 M.'+
								'</div>\
							</div>\
							<div id="item-'+m+'-2" class="small item conditional-format" data-condition="'+monster.vars.unaffordable10+'" data-format-class="unaffordable" onClick="Lolipop.interact(this);" data-lolipop=action:'+monster.vars.actionBuy10+'>\
								<div class="filler">10</div>\
								<div class="title gamedata" data-format-function="ultraAbbreviated" data-id="'+ monster.vars.price10 +'">'
									+'156 M.'+
								'</div>\
							</div>\
							<div id="item-'+m+'-2" class="small item refund" onClick="Lolipop.interact(this);" data-lolipop=action:'+monster.vars.actionRefund+'>\
								<div class="filler">1</div>\
								<div class="title gamedata" data-format-function="ultraAbbreviated" data-id="'+ monster.vars.refund +'">\
									ERR\
								</div>\
							</div>\
							<div id="item-'+m+'-2" class="small item info">\
								<div class="filler">/s</div>\
								<div class="title gamedata" data-format-function="ultraAbbreviated" data-id="'+ monster.vars.baseGps +'">\
									ERR\
								</div>\
							</div>\
							<div id="item-'+m+'-2" class="small item info">\
								<div class="filler"><small>all</small>/s</div>\
								<div class="title gamedata" data-format-function="ultraAbbreviated" data-id="'+ monster.vars.gps +'">\
									ERR\
								</div>\
							</div>\
						</div></div></div>\
				...
				$('#item-'+m+' > .thumbnail').click(function(){

					console.log('CLICK');
					//$(this).children('.menu').toggleClass('display-none');
					//$('#'+this.id+'-1,#'+this.id+'-2').toggleClass('display-none');
					$(this).parent().siblings().removeClass('selected');
					$(this).parent().toggleClass('selected');
				});

				container.find('.adjust-reveal')

				setTimeout(function(){ container.removeClass('hide-right')}, 1);
			}
		},
		close: function(){
			$('#monster-list').remove();
	...
	Interface.View.list['enchants-list'] = {
		open: function(){
			console.log('Opening enchants-list');
			$('#windows-insertion').append($("\
				<div id='enchants-list' class='menu glyph-menu horizontal-hide hide-right'></div>\
			"));
			var container = $('#enchants-list');

			// HERE cepocepo
			container.append($('<div class="title">Available</div>'));

			console.log(Upgrade.unlocked);

			for( var key in Upgrade.unlocked){
				var u = Upgrade.unlocked[key];
				var color = key.substring(1).hexEncode().substring(2);
				console.log(key, key[0], key.substring(1), key.substring(1).hexEncode().substring(0,6));
				container.append($('<div class="item" style="background-color: #'+color+';">\
					<div class="glyph">'+key[0]+'</div>\
					<div class="title">'+u.shop.name+'</div>\
					</div>'));
			}

			['a','b','c','d','e','f','g','h','i','j','k','l','m'].map(function( c ){
				container.append($('<div class="item">\
					<div class="glyph">'+c+'</div>\
					<div class="title">Thick Furr</div>\
					</div>'));
			});
			
			container.append($('<div class="title">Performed</div>'));
	...
	$(document).ready(function(){
		...
		*/
		...
		function raiseVisualMoney( x, y ){

			var str = Data.Formatting.Functions.abbreviatedNumber(Data.value('game.gold.gpc'));
			var w = parseInt($('body').css('width')), deviation = (Math.random() - 0.5)/4;
			...
			console.log('EFX', x, w, deviation);

			setTimeout(function(){
				$div.css('top',
					parseInt($div.css('top'))/3);
				$div.css('opacity',0);
			}, 10);
		...
		$('#main-button').on('touchstart', function( e ){
			...
			if( Data.updateVar('action.click.mick') ){
		...
		$('#main-button').on('click', function(e){
			if( Data.updateVar('action.click.mick') ){
				raiseVisualMoney(e.pageX, e.pageY);
			}
		});
...
$(document).ready(function(){
	...
	//$('#main-menu > [data-lolipop="view:monster-list"]').click();
