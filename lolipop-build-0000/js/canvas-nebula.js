...
$(document).ready(function(){	
	...
	(function ($) {			
			...
			var canvasResX = parseInt($canvas.attr('width'));
			var canvasResY = parseInt($canvas.attr('height'));

			var canvas2W = parseInt($canvas2.attr('width')), canvas2H = parseInt($canvas2.attr('height'));
			console.log(canvas2W,canvas2H);
			...
			var	Puff = function(p) {				
				var	opacity,
					sy = (Math.random()*canvasResY)>>0,
					sx = (Math.random()*canvasResX)>>0;
				...
				this.move = function(timeFac) {						
					...
					ctx.drawImage($canvas3[0], sy+p, sy+p, canvasResY-(p*2),canvasResX-(p*2), 0,0, w, h);								
				};
			};
			...
			{								
				...
				ctx2.drawImage( $canvas[0] ,0,0,canvas2W,canvas2H);				
				setTimeout(loop, 10 );				
			};
			...
			$(img).bind('load',null, function() {  ctx3.drawImage(img, 0,0, canvas2W, canvas2H);	loop(); });
